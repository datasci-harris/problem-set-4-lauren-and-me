...
```{python}
geometry=texas_data
merge_closures=geometry.merge(closures_by_zip_texas, left_on="ZCTA5", right_on='ZIP_CD')
merge_closures.head(15)
```

```{python}
texas_closures=merge_closures.plot(column='Closures', legend=True).set_axis_off()
test=texas_data.plot(facecolor='none', linewidth=0.2).set_axis_off()
test
```
3. 
4. 
