...
texas_data=texas_data[['ZCTA5', 'geometry']]

```

```{python}
zip_counts=short_term_hos_2016['ZIP_CD'].value_counts().reindex(short_term_hos_2016.ZIP_CD.unique(), fill_value=0)
texas_data.dtypes
zip_counts=zip_counts.reset_index()
zip_counts.dtypes
zip_counts['ZIP_CD']=zip_counts['ZIP_CD'].map(NotImplementedError)
```
```{python}
merged=texas_data.merge(zip_counts, left_on="ZCTA5", right_on="ZIP_CD", how='left')
merged.head()
merged.fillna(0)
```
```{python}
import matplotlib
counts=merged.plot(column='count', legend=True, linewidth=0.2, vmin=0).set_axis_off()
counts
```
